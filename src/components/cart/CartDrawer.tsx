...
export const CartDrawer: React.FC<CartDrawerProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end bg-black/40 backdrop-blur-xs animate-in fade-in duration-200">
      <div
        className="w-full max-w-md bg-white text-slate-900 h-full flex flex-col justify-between border-l border-slate-200 p-6 animate-in slide-in-from-right duration-300"
        role="dialog"
        aria-modal="true"
      ...
      >
        ...
        <div className="flex items-center justify-between pb-4 border-b border-slate-200">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-xl font-bold tracking-wide text-slate-900">Your Cart</h2>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto py-4 flex flex-col gap-4">
          {items.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center text-center gap-3 text-slate-500 my-auto">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 mt-2">
                    <button
                      ...
                      className="w-6 h-6 rounded-full bg-slate-200 flex items-center justify-center text-slate-700 hover:bg-slate-300 transition-colors"
                    ...
                    <button
                      ...
                      className="w-6 h-6 rounded-full bg-slate-200 flex items-center justify-center text-slate-700 hover:bg-slate-300 transition-colors disabled:opacity-30"
        ...
        {items.length > 0 && (
          <div className="pt-4 border-t border-slate-200 flex flex-col gap-4">
            ...
            <button
              onClick={clearCart}
              className="text-xs text-slate-500 hover:text-red-500 transition-colors text-center py-1 cursor-pointer"
            >
              Clear Cart
            </button>
          </div>
